...
class ExactInference(InferenceModule):
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        allPossible = util.Counter()
        for p in self.legalPositions:
            newDistribution = self.getPositionDistribution(self.setGhostPosition(gameState, p))

            for newPosn, probability in newDistribution.items():
                allPossible[newPosn] += probability * self.beliefs[p]
...
class ParticleFilter(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance == None:
            ...
            for p in self.particles:
                trueDistance = util.manhattanDistance(p, pacmanPosition)
                temp[p] += emissionModel[trueDistance]
            beliefs = temp
            beliefs.normalize()

        if beliefs.totalCount():
            t = []
            for i in range(len(self.particles)):
                t.append(util.sample(beliefs))
            self.particles = t
        else:
            self.initializeUniformly(gameState)
    ...
    def elapseTime(self, gameState):
        ...
        for p in self.particles:
            ...
            newP.append(util.sample(newPosDist))
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        "*** YOUR CODE HERE ***"
        pos = self.legalPositions
        perm = list (itertools.product(pos, repeat=self.numGhosts))
        numPart = self.numParticles
